...
# Start of the script
echo
echo "${GREEN_TEXT}${BOLD_TEXT}Starting the process...${RESET_FORMAT}"
echo

GCP_PROJECT_ID=$(gcloud config get-value project)
echo "${BLUE_TEXT}${BOLD_TEXT}Using GCP Project ID: ${GREEN_TEXT}$GCP_PROJECT_ID${RESET_FORMAT}"

# Enable the BigQuery Migration API
echo
echo "${YELLOW_TEXT}${BOLD_TEXT}Enabling the BigQuery Migration API...${RESET_FORMAT}"
...
echo
echo "${CYAN_TEXT}${BOLD_TEXT}Creating a Google Cloud Storage bucket...${RESET_FORMAT}"
